...
fn intel_amd_detect(vendor: Vendor, max_basic: usize, max_extended: usize) -> FeatureDetect {
    ...
    let has_x2apic = (cpuid1.ecx >> 21) & 1 == 1;
    ...
    if !has_x2apic {
        return FeatureDetect::Insufficient(InsufficientReason::Nox2Apic);
    }
    if !has_rdrand {
        return FeatureDetect::Insufficient(InsufficientReason::NoRdRand);
    }
